...
def wedge(a, b, t):
    ...
    i = float(
        integrator.integrate(
            ...
            dim=2,
            N=999999,
            integration_domain=[[-np.sqrt(t**2 - b**2), np.sqrt(t**2 - b**2)], [a, h]],
        )
    )
    return i
...
def semi_wedge(a, b, c, t):
    ...
    i = float(
        integrator.integrate(
            f, dim=2, N=999999, integration_domain=[[-np.sqrt(t**2 - b**2), c], [a, h]]
        )
    )
    return i
...
def c_i(t, pos):
    out = 0.0
    faces = [255 - pos[0], pos[0], 255 - pos[1], pos[1], 255 - pos[2], pos[2]]
    for a in faces:
        if a < t:
            out += cap(a, t)
    return out
...
for model in MODELS:
    print(
        f"{model}: {abs(1-Uniformity(mistakes[model],distr)/Uniformity(total[model],distr))}"
    )
