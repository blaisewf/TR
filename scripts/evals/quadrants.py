...
def quadrats(res, points):
    ...
    for point in points:
        cuboids[
            int(
                (np.ceil(point[0] * res / 255) - 1)
                + (np.ceil(point[1] * res / 255) - 1) * res
                + (np.ceil(point[2] * res / 255) - 1) * res**2
            )
        ] += 1
...
mistakes = {
    model: (np.array(data)[:, 0] + np.array(data)[:, 1]) / 2
    for model, data in mistakes.items()
}
total = {
    model: (np.array(data)[:, 0] + np.array(data)[:, 1]) / 2
    for model, data in total.items()
}
